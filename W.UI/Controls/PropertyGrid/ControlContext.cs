...
namespace W.UI.Controls
{
    ...
    public class LayoutProcessor : IControlProcessor
    {
        ...
        private readonly TitlePlacementType _direction;

        // 可以在构造函数中传入布局方向，或者从特性中读取
        public LayoutProcessor(SuperDisplayAttribute display, TitlePlacementType direction = TitlePlacementType.Left)
        {
            _display = display;
        ...
        public void Execute(ControlContext context)
        {
            ...
            switch (_direction)
            {
                ...
                case TitlePlacementType.Bottom:
                    // 标签在下：[Control]
                    //          [Label]
                    grid.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
                    grid.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
        ...
        private TextBlock CreateLabel(PropertyInfo prop, SuperDisplayAttribute display)
        {
